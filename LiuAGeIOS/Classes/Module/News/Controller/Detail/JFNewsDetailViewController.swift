...
fileprivate let detailOtherLinkNoneIdentifier = "detailOtherLinkNoneIdentifier"
fileprivate let detailCommentIdentifier = "detailCommentIdentifier"
fileprivate let detailNewsAdIdentifier = "detailNewsAdIdentifier"
...
class JFNewsDetailViewController: UIViewController {
    ...
    fileprivate var model: JFArticleDetailModel? {
        didSet {
            ...
            // 更新收藏状态
            if model.havefava == "1" && JFAccountModel.isLogin() {
                self.bottomBarView.collectionButton.isSelected = true
            } else {
                self.bottomBarView.collectionButton.isSelected = false
            }
            
            // 相关链接
            if var links = model.otherLinks {
                // 相关链接插入广告
                if otherLinks.count == 0 && JFAdManager.shared.articleList.count > 0 {
                    let adRandNum1 = Int(arc4random_uniform(UInt32(JFAdManager.shared.articleList.count)))
                    let otherRandNum = Int(arc4random_uniform(UInt32(links.count)))
                    let adModel = JFAdManager.shared.articleList[adRandNum1]
                    let otherLink = JFOtherLinkModel(dict: [
                        "classid" : adModel.classid ?? "",
                        "id" : adModel.id ?? "",
                        "titlepic" : adModel.titlepic ?? "",
                        "onclick" : adModel.onclick ?? "",
                        "title" : adModel.title ?? "",
                        "classname" : adModel.classname ?? "",
                        "isurl" : adModel.isurl ?? "",
                        "titleurl" : adModel.titleurl ?? ""
                        ])
                    links.insert(otherLink, at: otherRandNum)
                    otherLinks = links
                }
            }
    ...
    /// banner广告模型
    fileprivate var bannerModel: JFArticleListModel?
    /// 是否已经加载过webView
    ...
    /// 加载banner广告模型
    fileprivate func loadBannerModel() {
        // banner广告 - 一定是带图片的
        var tempAdList = [JFArticleListModel]()
        for article in JFAdManager.shared.articleList {
            if article.titlepic != nil && article.titlepic != "" {
                tempAdList.append(article)
            }
        }
        if tempAdList.count > 0 {
            let adRandNum2 = Int(arc4random_uniform(UInt32(tempAdList.count)))
            bannerModel = tempAdList[adRandNum2]
        }
    }
    ...
    /**
     ...
     - parameter id:      文章id
     */
    func loadNewsDetail(_ classid: String, id: String) {
    ...
    fileprivate lazy var tableView: UITableView = {
        ...
        tableView.register(UINib(nibName: "JFNewsAdCell", bundle: nil), forCellReuseIdentifier: detailNewsAdIdentifier)
        tableView.tableHeaderView = self.webView
        tableView.tableFooterView = self.closeDetailView
        return tableView
    }()
    ...
    fileprivate lazy var footerView: UIView = {
        ...
        moreCommentButton.backgroundColor = ACCENT_COLOR
...
extension JFNewsDetailViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0: // 分享
            let cell = tableView.dequeueReusableCell(withIdentifier: detailStarAndShareIdentifier) as! JFStarAndShareCell
            cell.delegate = self
            cell.selectionStyle = .none
            return cell
        case 1: // 广告
            let cell = tableView.dequeueReusableCell(withIdentifier: detailNewsAdIdentifier) as! JFNewsAdCell
            cell.selectionStyle = .none
        ...
        case 2: // 相关阅读
            ...
            if model.titlepic == nil || model.titlepic == "" {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 2 || section == 3 {
            ...
            leftView.backgroundColor = PRIMARY_COLOR
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case 0: // 分享
            return 99
        case 1: // 广告
            return JFAdManager.shared.articleList.count == 0 ? 0.1 : 180
        case 2: // 相关阅读
            return 100
        case 3: // 评论
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 1 { // 广告
            if let model = bannerModel {
                // 外部链接，则跳转到网页
                if model.classid == JFAdManager.shared.classid && model.isurl == "1" {
                    let adWebViewVc = JFAdWebViewController()
                    adWebViewVc.webParam = (model.title!, model.titleurl!, model.titlepic)
                    navigationController?.pushViewController(adWebViewVc, animated: true)
                    return
                }
                
                let detailVc = JFNewsDetailViewController()
                detailVc.articleParam = (model.classid!, model.id!)
                navigationController?.pushViewController(detailVc, animated: true)
            }
        } else if indexPath.section == 2 { // 相关链接
            
            let otherModel = otherLinks[indexPath.row]
            
            // 外部链接，则跳转到网页
            if otherModel.classid == JFAdManager.shared.classid && otherModel.isurl == "1"  {
                let adWebViewVc = JFAdWebViewController()
                adWebViewVc.webParam = (otherModel.title!, otherModel.titleurl!, otherModel.titlepic)
                navigationController?.pushViewController(adWebViewVc, animated: true)
                return
            }
    ...
    func didTappedCollectButton(_ button: UIButton) {
        ...
        if JFAccountModel.isLogin() {
            ...
            JFNetworkTool.shareNetworkTool.post(ADD_DEL_FAVA, parameters: parameters) { (success, result, error) in
                ...
                guard let successResult = result, success == true else {return}
                ...
                JFNewsDALManager.shareManager.removeNewsDetail(classid: self.articleParam?.classid ?? "", id: self.articleParam?.id ?? "")
            }
        } else {
    ...
    func didTappedSendButtonWithMessage(_ message: String) {
        ...
        JFNetworkTool.shareNetworkTool.get(SUBMIT_COMMENT, parameters: parameters) { (success, result, error) in
            ...
            if success {
                ...
                if let info = result?["result"].string {
                    JFProgressHUD.showInfoWithStatus(info)
                }
            }
...
extension JFNewsDetailViewController: JFSetFontViewDelegate {
    ...
    func didChangedFontName(_ fontName: String) {
        ...
        UserDefaults.standard.set(fontName, forKey: CONTENT_FONT_TYPE_KEY)
        autolayoutWebView()
    }
    ...
    func didChangedNightMode(_ on: Bool) {
        UserDefaults.standard.set(on, forKey: NIGHT_KEY)
...
extension JFNewsDetailViewController: JFStarAndShareCellDelegate, JFShareViewDelegate {
    ...
    func getShareParameters() -> NSMutableDictionary? {
        ...
        // 分享图片
        var image = YYImageCache.shared().getImageForKey(model?.titlepic ?? "")
        ...
        if smalltext.characters.count > 25 {
            smalltext = (smalltext as NSString).substring(with: NSMakeRange(0, 25))
        }
        
        let shareParames = NSMutableDictionary()
        shareParames.ssdkSetupShareParams(byText: smalltext,
                                          images : image,
...
extension JFNewsDetailViewController: JFCommentCellDelegate {
    ...
    /**
     加载评论信息 - 只加载最新的10条
     */
    func loadCommentList(_ classid: String, id: String) {
    ...
    /**
     点击更多评论按钮
     */
    func didTappedmoreCommentButton(_ button: UIButton) {

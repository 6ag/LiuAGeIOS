...
class JFPhotoDetailViewController: UIViewController {
    ...
    var model: JFArticleDetailModel? {
        didSet {
            ...
            // 更新评论数量
            if model?.plnum != "0" {
                self.bottomToolView.commentButton.isHidden = false
                self.bottomToolView.commentButton.setTitle(model?.plnum ?? "", for: .normal)
            } else {
                self.bottomToolView.commentButton.isHidden = true
            }
            
            // 更新收藏状态
            if model?.havefava == "1" && JFAccountModel.isLogin() {
                self.bottomToolView.collectionButton.isSelected = true
            } else {
                self.bottomToolView.collectionButton.isSelected = false
            }
    ...
    /**
     ...
     - parameter id:      文章id
     */
    func loadPhotoDetail(_ classid: String, id: String) {
    ...
    fileprivate lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(JFPhotoDetailCell.classForCoder(), forCellWithReuseIdentifier: self.photoIdentifier)
    ...
    func didTappedCollectButton(_ button: UIButton) {
        ...
        if JFAccountModel.isLogin() {
            ...
            JFNetworkTool.shareNetworkTool.post(ADD_DEL_FAVA, parameters: parameters) { (success, result, error) in
                ...
                JFNewsDALManager.shareManager.removeNewsDetail(classid: self.photoParam?.classid ?? "", id: self.photoParam?.id ?? "")
            }
        } else {
    ...
    func didTappedSendButtonWithMessage(_ message: String) {
        ...
        JFNetworkTool.shareNetworkTool.get(SUBMIT_COMMENT, parameters: parameters) { (success, result, error) in
            if success {
                ...
                // 评论数量自增1
                if let num = self.model?.plnum {
                    var count = Int(num)!
                    count += 1
                    self.bottomToolView.commentButton.setTitle("\(count)", for: .normal)
                }

...
class JFEditColumnViewController: UIViewController, JFEditColumnViewCellDelegate {
    ...
    // MARK: - 各种排序处理方法
    func sortItem(_ pan: UIPanGestureRecognizer) {
        ...
        if pan.state == UIGestureRecognizerState.began {
            ...
            for (index, _) in selectedCategoryList.enumerated() {
            ...
            if rect.contains(CGPoint(x: pan.view!.center.x, y: pan.view!.center.y)) && cellIndexPath != attributes.indexPath {
                if cellIndexPath?.row ?? 0 > attributes.indexPath.row {
                    ...
                    for index in ((attributes.indexPath.row + 1)...cellIndexPath!.row).reversed() {
                        let temp = selectedCategoryList[index]
                        selectedCategoryList[index] = selectedCategoryList[index - 1]
                        selectedCategoryList[index - 1] = temp
                    }
                } else {
                    //前面跟后面交换
    ...
    func deleteItemWithIndexPath(_ indexPath: IndexPath) {
        optionalCategoryList.insert(selectedCategoryList[indexPath.row], at: 0)
        selectedCategoryList.remove(at: indexPath.row)
        collectionView.deleteItems(at: [indexPath])
        
        // 删除之后更新collectionView上对应的indexPath
        for index in 0 ..< selectedCategoryList.count {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            selectedCategoryList.append(optionalCategoryList[indexPath.row])
            ...
            animationLabel.text = optionalCategoryList[indexPath.row].classname ?? ""
            collectionView.addSubview(animationLabel)
            let toIndexPath = IndexPath(item: selectedCategoryList.count-1, section: 0)
            ...
            UIView.animate(withDuration: 0.7, animations: { 
                self.animationLabel.center = endAttributes!.center
                    ...
                    self.optionalCategoryList.remove(at: indexPath.row)
...
extension JFEditColumnViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if section == 0 {
            ...
            return optionalCategoryList.count
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            cell.configCell(selectedCategoryList, withIndexPath: indexPath)
            ...
            if indexPath.row == 0 {
                ...
                //最后一位是否影藏(为了动画效果)
                if indexPath.row == selectedCategoryList.count - 1 {
                    cell.contentLabel.isHidden = lastIsHidden
                }
            }
            
        } else {
            cell.configCell(optionalCategoryList, withIndexPath: indexPath)
            cell.deleteButton.isHidden = true
        }

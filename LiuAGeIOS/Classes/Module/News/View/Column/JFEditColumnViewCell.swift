...
class JFEditColumnViewCell: UICollectionViewCell {
    ...
    /**
     配置cell
     */
    func configCell(_ categoryList: [JFCategoryModel], withIndexPath indexPath: IndexPath) -> Void {
        self.indexPath = indexPath
        contentLabel.isHidden = false
        contentLabel.text = categoryList[indexPath.row].classname ?? ""
        
        if (indexPath.section == 0 && indexPath.row == 0) {
            contentLabel.textColor = ACCENT_COLOR
    ...
    /**
     点击了删除按钮
     */
    func didTappedDeleteButton(_ button: UIButton) -> Void {
